"""
    ...
    Given a list of emails, we send one email to each address in 
    the list.  How many different addresses actually receive mails? 

    Example 1:  Input: ["test.email+alex@leetcode.com","test.e.mail+bob.cathy@leetcode.com","testemail+david@lee.tcode.com"]
    Output: 2
...
class Solution:
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            temp = email.split('@')
            if temp[0].find('+') != -1:
                temp[0] = temp[0][:temp[0].find('+')]
            temp[0] = temp[0].replace('.', '')
            email_set.add('@'.join(temp))
