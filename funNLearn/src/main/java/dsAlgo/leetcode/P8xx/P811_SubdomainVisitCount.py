"""
    ...
    of visits this domain received), followed by a space, followed by the address. 
    An example of a count-paired domain might be "9001 discuss.leetcode.com".

    We are given a list cpdomains of count-paired domains. We would like a 
    list of count-paired domains, (in the same format as the input, 
    and in any order), that explicitly counts the number of visits to 
    each subdomain.
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpd in cpdomains:
            dc = int(cpd.split(' ')[0])
            dmn = cpd.split(' ')[1].split('.')
            for i in range(len(dmn)):
                cntr['.'.join(dmn[i:])] += dc
        return ['{} {}'.format(cntr[x], x) for x in cntr]
