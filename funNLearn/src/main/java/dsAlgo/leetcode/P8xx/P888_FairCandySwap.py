...
class Solution:
    def fairCandySwap(self, A: List[int], B: List[int]) -> List[int]:
        candy_diff, setB = (sum(B) - sum(A)) / 2, set(B)
        for x in A:
            if x + candy_diff in setB:
                return [x, x + candy_diff]


assert Solution().fairCandySwap(A=[1, 1], B=[2, 2]) == [1, 2]
assert Solution().fairCandySwap(A=[1, 2], B=[2, 3]) == [1, 2]
assert Solution().fairCandySwap(A=[2], B=[1, 3]) == [2, 3]
assert Solution().fairCandySwap(A=[1, 2, 5], B=[2, 4]) == [5, 4]
print('Tests Passed!!')
