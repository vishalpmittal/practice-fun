...
class Solution:
    def shortestCompletingWord(self, licensePlate: str, words: List[str]) -> str:
        ...
        for c in licensePlate:
            if c.isalpha():
                cl = c.lower()
                lpol[cl] = 1 if cl not in lpol else lpol[cl]+1
        ...
        for w in words:
            ...
            for c in w:
                if c in lpol:
                    wo[c] = 1 if c not in wo else wo[c]+1
            if wo == lpol and len(w) < len(ans):
                ans = w

        return ans

    def shortestCompletingWord_arr(self, licensePlate, words):
        def count(itera):
            ans = [0] * 26
            for letter in itera:
