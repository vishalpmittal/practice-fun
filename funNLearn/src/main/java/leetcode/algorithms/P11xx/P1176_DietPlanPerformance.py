"""
    ...
    days (calories[i], calories[i+1], ..., calories[i+k-1] for 
    all 0 <= i <= n-k), they look at T, the total calories 
    ...
    Initially, the dieter has zero points. Return the total 
    number of points the dieter has after dieting for calories.length days.

    Note that the total points can be negative.

    Example 1: Input: calories = [1,2,3,4,5], k = 1, lower = 3, upper = 3
    Output: 0
...
class Solution:
    def dietPlanPerformance(self, C: List[int], k: int, lower: int, upper: int) -> int:
...
assert Solution().dietPlanPerformance([3, 2], 2, 0, 1) == 1
assert Solution().dietPlanPerformance([6, 5, 0, 0], 2, 1, 5) == 0
print('Tests Passed!!')
