...
```bash
   ...
   $ virtualenv -p `which python` vijango  # create virtualenv
   $ . vijango/bin/activate                # activate virtualenv
   $ pip isntall django                    # install django
```
...
- models needs to be added to be viewed here. Register Model for admin access
  -  in <app_folder> -> admin.py -> admin.site.register(Album)

- admin commands:
```bash
> django-admin startproject MyProject        # Start a new Project
> django-admin startapp accounts        # Start a new Project
```
