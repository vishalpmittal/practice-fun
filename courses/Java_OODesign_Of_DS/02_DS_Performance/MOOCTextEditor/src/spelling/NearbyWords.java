...
public class NearbyWords implements SpellingSuggest {
		...
		for (int index = 0; index <s.length()+1; index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				StringBuffer sb = new StringBuffer(s.substring(0, index) 
						+ (char)charCode + s.substring(index));
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (queue.size()>0 && retList.size()<numSuggestions && visited.size() < THRESHOLD){
			...
			List<String> oneDistSuggList = distanceOne(curr, true);
			for (String oneDistSuggWord : oneDistSuggList){

				if (retList.size() >= numSuggestions || visited.size() >= THRESHOLD)
					break;
				
				if (!visited.contains(oneDistSuggWord)){
					visited.add(oneDistSuggWord);
					queue.add(oneDistSuggWord);
					if (this.dict.isWord(oneDistSuggWord)){
						retList.add(oneDistSuggWord);
					}
				}
			}
		}
   ...
   public static void main(String[] args) {
	   ...
	   List<String> retList = new ArrayList<String>();
	   w.deletions("hello", retList, true);
	   */
