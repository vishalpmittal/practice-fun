...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			String readChars = getStringFromFile(textfile, numToCheck);
			
			long startTime = System.nanoTime();
			for (int trial = 0; trial < trials; trial++) {
				BasicDocument bd = new BasicDocument(readChars);
				bd.getFleschScore();
			}
			long endTime = System.nanoTime();
			System.out.print(((endTime-startTime)/Math.pow(10, 9)) + "\t");

			startTime = System.nanoTime();
			for (int trial = 0; trial < trials; trial++) {
				EfficientDocument bd = new EfficientDocument(readChars);
				bd.getFleschScore();
			}
