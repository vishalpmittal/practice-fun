...
def hierarchical_clustering(cluster_list, num_clusters):
    ...
    while len(clusters) > num_clusters:
        ...
        min_dist_tuple = fast_closest_pair(clusters)

        clus_one = clusters[min_dist_tuple[1]].copy()
        del_one = clusters[min_dist_tuple[1]]
        clus_two = clusters[min_dist_tuple[2]].copy()
        del_two = clusters[min_dist_tuple[2]]

        new_cluster = clus_one.merge_clusters(clus_two)

        clusters.remove(del_one)
        clusters.remove(del_two)
...
# print hierarchical_clustering(my_cl_list, 4)