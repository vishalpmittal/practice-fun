...
# Create ELB
def create_elb(lb_name, lg_inst_dns, lb_s_grp_obj):
    regions = boto.ec2.elb.regions()
    elb_conn = regions[0].connect()
    ...
    lb = elb_conn.create_load_balancer(lb_name, zones, listeners=ports, security_groups=[lb_s_grp_obj.id])
...
def create_ASG(lb_name, l_config, min_asg_size, max_asg_size, desired_asg_size):
    as_conn = AutoScaleConnection()
    ...
    asg = AutoScalingGroup(group_name=asg_name, load_balancers=[lb_name],
                          availability_zones= zones, desired_capacity=desired_asg_size,
...
def create_scaleup_policy(cooldown_time):
    as_conn = AutoScaleConnection()
    scale_up_policy = ScalingPolicy(name='scale_up', adjustment_type='ChangeInCapacity', as_name= asg_name, scaling_adjustment=1, cooldown=cooldown_time)
    as_conn.create_scaling_policy(scale_up_policy)
    return scale_up_policy

# Define scale down policies
def create_scaledown_policy(cooldown_time):
    as_conn = AutoScaleConnection()
    scale_down_policy = ScalingPolicy(name='scale_down', adjustment_type='ChangeInCapacity',as_name= asg_name, scaling_adjustment=-1, cooldown=cooldown_time)
...
def delete_all_resources():
    ...
    asg = AutoScalingGroup(group_name=asg_name, load_balancers=[lb_name],
                          ...
                          launch_config=launch_config_name, min_size=0, max_size=0,
                          connection=as_conn, health_check_type ='ELB', health_check_period='60')
    asg.update()
    
    # wait for 2 mins for all instances to kill
    print "Sleeping for a mins for instance termination"
    time.sleep(60)
    ...
    print "Removed all the Resourses except Load Generator"
...
def main():
    ...
    print "creating ELB"
    lg_elb = create_elb(lb_name, lg_inst_dns, lb_s_grp_obj)
    ...
    up_watch_period = '60'
    up_watch_eval_period = 2

    down_watch_threshold = '55'
    down_watch_period = '60'
    down_watch_eval_period = 4
    ...
    print "creating ASG "
    asg = create_ASG(lb_name, lc, min_asg_size, max_asg_size, desired_asg_size)
    ...
    sup_pol= create_scaleup_policy(cooldown_time)
    sdown_pol = create_scaledown_policy(cooldown_time)
    ...
    ############################################################
               Wait for DC instances to come up
    ############################################################
    for i in range (0,2):
        time.sleep(70)
        refresh_url= "http://"+lg_inst_dns
        print refresh_url
        refresh_response=openUrl(refresh_url)
        print refresh_response

    #############################################################
    #            run warmup
    #############################################################
    lg_inst_dns="ec2-54-152-168-175.compute-1.amazonaws.com"
